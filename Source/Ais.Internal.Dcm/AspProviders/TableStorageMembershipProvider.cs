    ...
    public class MembershipRow : TableServiceEntity, IComparable
    {
        ...
        public string CompanyName
        {
            set
            {
                if (value == null)
                {
                    throw new ArgumentException("Company Name shoud not be empty");
                }
                _companyName = value;
            }
    ...
    public class TableStorageMembershipProvider : MembershipProvider
    {
        ...
        public override MembershipUserCollection FindUsersByName(string usernameToMatch, int pageIndex, int pageSize, out int totalRecords)
        {
            ...
            try
            {
                ...
                CloudTableQuery<MembershipRow> query;
                CloudTableQuery<MembershipRow> refinedQuery;
                ...
                else if (startswith)
                {
                    ...
                    query = (from user in queryObj
                             where user.PartitionKey.CompareTo(SecUtility.EscapedFirst(_applicationName)) > 0 &&
                                   user.PartitionKey.CompareTo(SecUtility.NextComparisonString(SecUtility.EscapedFirst(_applicationName))) < 0 &&
                                   user.Email.CompareTo(usernameToMatch) == 0 &&
                                   user.ProfileIsCreatedByProfileProvider == false
                             select user).AsTableServiceQuery();
                } else {
                    query = (from user in queryObj
                ...
                for (i = startIndex; i < endIndex && i < allUsersList.Count && userMatches; i++) {
                    ...
                    if (startswith && !string.IsNullOrEmpty(usernameToMatch))
                    {
                        if (!row.Email.StartsWith(usernameToMatch, StringComparison.Ordinal))
                        {
                            userMatches = false;
                            continue;
                        }
        ...
        bool IsMatch(MembershipRow user, string usernameToMatch)
        {
            string userToCompare = user.UserName.Split('@')[1];
            return string.Compare(userToCompare, usernameToMatch, true) == 0;
        }
        /// <summary>
        ...
        private string EncodePassword(string pass, int passwordFormat, string salt)
        {
            ...
            { // MembershipPasswordFormat.Hashed
                using (HashAlgorithm s = HashAlgorithm.Create(Membership.HashAlgorithmType))
                {
                    ...
                    {
                        ((KeyedHashAlgorithm)s).Key = bSalt;
                    }
